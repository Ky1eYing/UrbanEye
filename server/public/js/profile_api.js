...
async function getUserLikes(userId) {
    try {
        ...
        if (!response.ok) {
            console.error(`Error fetching user likes (${response.status}): ${response.statusText}`);
            return null;
        }
        ...
        if (data.code === 200 && data.data) {
            console.log(`Successfully fetched ${data.data.length} liked events for user`);
            return data.data;
        } else {
            console.error('Error in API response:', data.message || 'Unknown error');
            return null;
...
async function addUserLike(userId, eventId) {
    try {
        ...
        if (!response.ok) {
            console.error(`Error adding like (${response.status}): ${response.statusText}`);
            return false;
        }
...
async function removeUserLike(userId, eventId) {
    try {
        ...
        if (!response.ok) {
            console.error(`Error removing like (${response.status}): ${response.statusText}`);
            return false;
        }
...
async function getUserComments(userId) {
    try {
        ...
        if (!response.ok) {
            console.error(`Error fetching user comments (${response.status}): ${response.statusText}`);
            return null;
        }
...
async function updateUserPassword(userId, originalPassword, newPassword) {
    try {
        if (!userId || !originalPassword || !newPassword) {
            console.error('Error: User ID, original password, and new password are required');
        ...
        if (!response.ok) {
            ...
            console.error(`Error updating password (${response.status}):`, errorData.message);
