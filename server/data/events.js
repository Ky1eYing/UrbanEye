...
const getEventByEventId = async eventId => {
	...
	const event = await eventsCollection.findOne({ _id: new ObjectId(checked_eventId) });
	if (!event) {
...
const getEventByUserId = async userId => {
	...
	const eventsCollection = await events();
	let userEvents = await eventsCollection.find({ user_id: new ObjectId(checked_userId) }).toArray();
...
const removeEvent = async eventId => {
	...
	const eventsCollection = await events();
	const deletionInfo = await eventsCollection.findOneAndDelete({ _id: new ObjectId(checked_eventId) });
	if (!deletionInfo) {
...
const updateEvent = async (event_id, title, content, location, category, photoUrl) => {
	...
	if (!updateInfo) {
		throw new Error(`Error: Update failed, could not find an event with id of ${checked_eventId}`);
	}
