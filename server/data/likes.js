...
const createLike = async (user_id, event_id) => {
    ...
    const event = await eventsCollection.findOne({ _id: new ObjectId(checked_event_id) });
    if (!event) {
        throw new Error("Event not found");
    }

    // Check if user already liked this event
    const userAlreadyLiked = event.likes.some(like => like.user_id.toString() === checked_user_id);
    if (userAlreadyLiked) {
...
const getLikeByUserId = async (user_id) => {
    ...
    const likedEvents = eventsList.map(event => {
        const userLike = event.likes.find(like => 
            like.user_id.toString() === checked_user_id
        );
